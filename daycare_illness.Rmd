...
The model fits a general trend line to our data that does not fit very well due to noise, it does capture the overall trend, however, it expects a faster build up in occupying the ARI (+) state (state 2, t=0 to t=10).
...
plyr::ddply(tmp, "id", fcor) %>%
  ...
  add(theme_trueMinimal()) %>%
  add(coord_flip()) %>%
  add(labs(y = "\nCorrelation, daycare vs. playgroup attendance"))
```
...
The baseline Weibull hazard function, $\lambda(t) = \alpha t^{\alpha - 1}/\gamma^\alpha$

```{r}
plot(recur.mod1, color = 1)
```

and survival function $S(t) = e^{-(t / \gamma)^\alpha}$
...
The second model provides a slightly better fit

Concordance measures

```{r}
mod2Cmeas <- Cmeasures(recur.mod2)

mod2Cmeas
```

Predictions can be done in two ways,

1. Marginal predictive probability of event between t and horizon time t + w, averaged    over the population:

$$P^{marg}(t, t+w)=\frac{\int_0^{+\infty}(S_{i(J+1)}(t|u_i) - S_{ij}(t+w|u_i)(u_i)^J S_{ij}(X_{iJ}|u_i)g(u)du}{\int_0^{+\infty}S_{i(J+1)}(t|u_i)(u_i)^J S_{i(J+1)}(X_{iJ}|u_i)g(u)du}$$
2. A conditional predictive probability of event between $t$ and horizon time $t+w$, give a specific individual.

$$P^{cond}(t, t+w) = \frac{S_{ij}(t|\eta_i) - S_{ij}(t+w|\eta_i)}{S_{ij}(t|\eta_i)} = 1 - \left\{\frac{S_0(t+w)}{S_0(t)} \right\}^{exp(\eta_i + \beta^{\prime}Z_{ij})}$$

```{r}
datapred <- data.frame(daycare = 1)
datapred$daycare <- as.factor(datapred$daycare)
levels(datapred$daycare) <- c(0, 1)
datapred[1, ] <- 0
datapred[2, ] <- 1
```
