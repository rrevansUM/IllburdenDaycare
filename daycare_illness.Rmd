...
```{r, echo = FALSE, eval = FALSE}
# setwd("S:/MartinEpi/Analysis/IllburdenDaycare/IllburdenDaycare/plots")
...
bootSojourn <- function(formula, 
  ...
  samp <- data[data[[grp]] %in% sample(levels(data[[grp]]), pergrp), ]
  samp <- arrange(samp, id, weeks)
...
```{r}
prev2 <- prevalence.msm(markov8, ci = "normal", B = 100, cores = 4)
```
...
```{r}
WAIC(brmmod1, brmmod6, compare = TRUE, cores = 4)
```
